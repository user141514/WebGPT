...
import { nearestConversationScrollContainer, stableMessageId, turnTestId } from './conversation-dom.js';
const MESSAGE_SELECTOR = '[data-message-author-role="user"],[data-message-author-role="assistant"]';
function numericTurnIndex(value) {
    if (!value)
        return null;
...
export function mergeConversationHistoryRecords(windows) {
    const byMessageId = new Map();
    let firstSeen = 0;
    const order = new Map();
    for (const window of windows) {
        for (const record of window) {
            if (!order.has(record.messageId))
                order.set(record.messageId, firstSeen++);
...
export function conversationHistoryWindow(document, knownMessageIds = new Set()) {
    const records = [];
    ...
    for (const element of [...document.querySelectorAll(MESSAGE_SELECTOR)]) {
        ...
        const testId = turnTestId(element);
        if (numericTurnIndex(testId) === null)
            missingTurnIndices += 1;
...
function toConversationTurns(records) {
    return records.map((record) => (record.role === 'user'
...
export async function scanConversationHistory(document, url, title, options = {}) {
    ...
    for (let round = 0; round < maxRounds; round += 1) {
        ...
        const afterTarget = (refreshedContainer ?? container);
