...
import { activateProjectCandidate, catalogProbeFromDocument, projectCandidatesFromDocument, scanCatalogDocument } from './catalog-dom.js';
import { ContentController } from './content-controller.js';
import { submitDriverMessage } from './content-submit.js';
const surface = new BrowserDomSurface(document);
const controller = new ContentController(surface, (envelope) => {
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message?.type === 'driver.catalog.project.open') {
        ...
        const exists = available.some((item) => (item.index === candidate.index && item.title === candidate.title)) || available.some((item) => item.title === candidate.title);
        if (!exists) {
