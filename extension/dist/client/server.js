...
export function resolveClientAsset(requestUrl, root = process.cwd()) {
...
export function createClientServer(options = {}) {
    ...
    const extensionControl = new Map();
    const driverCommands = new Map();
    const driverResults = new Map();
    const driverRelayClients = new Set();
    return createServer(async (request, response) => {
        ...
        if (url.pathname === '/__driver-control/next' && request.method === 'GET') {
            const now = Date.now();
            const next = [...driverCommands.entries()].find(([, command]) => (command.leasedAt === null || now - command.leasedAt > 5_000));
            if (!next) {
                response.writeHead(204);
                response.end();
                return;
        ...
        if (url.pathname === '/__live' && request.method === 'POST') {
            ...
            try {
                ...
                liveState = {
                    ...
                    historyMessages: Number.isInteger(payload.historyMessages) && payload.historyMessages >= 0
