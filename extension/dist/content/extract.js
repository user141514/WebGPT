...
function descendantsByTag(node, wanted) {
...
function texAnnotation(node) {
    const own = tag(node) === 'ANNOTATION' && attr(node, 'encoding') === 'application/x-tex'
        ? textContent(node).trim()
        : null;
    if (own)
        return own;
    const annotation = findDescendant(node, (candidate) => tag(candidate) === 'ANNOTATION'
        && attr(candidate, 'encoding') === 'application/x-tex');
    const latex = annotation ? textContent(annotation).trim() : '';
    return latex || null;
}
function classIncludes(node, token) {
    return (attr(node, 'class') ?? '').split(/\s+/).some((value) => value === token || value.includes(token));
}
function trimInlineEdges(content) {
    const copy = [...content];
    const trimStart = (inline) => {
        if (inline.type === 'text')
            return { ...inline, text: inline.text.replace(/^\s+/, '') };
        return inline;
    };
    const trimEnd = (inline) => {
        if (inline.type === 'text')
            return { ...inline, text: inline.text.replace(/\s+$/, '') };
        return inline;
    };
    ...
    if (copy.length)
        copy[0] = trimStart(copy[0]);
    if (copy.length)
        copy[copy.length - 1] = trimEnd(copy[copy.length - 1]);
    return copy;
}
function inlineFromNode(node) {
    ...
    if (nodeTag === 'SCRIPT' || nodeTag === 'STYLE' || nodeTag === 'BUTTON')
        return [];
    if (nodeTag === 'BR')
...
export function extractAssistantDocument(root) {
...
const TEXT_NODE = 3;
function isContentDomNode(node) {
    return node.nodeType === ELEMENT_NODE || node.nodeType === TEXT_NODE;
}
export function captureContentTree(node) {
