...
export class CatalogCoordinator {
  ...
  async refresh(seed?: ConversationCatalog, seedUrl?: string): Promise<ConversationCatalog> {
    if (!seed) {
      ...
      try {
        ...
        if (candidates.length) {
          for (const candidate of candidates) {
            ...
            catalog = await this.store.merge(mergeCatalogs(
              await this.browser.scan(tabId),
              {
    ...
    const projectUrls = [...projectSource]
      .sort((a, b) => a.title.localeCompare(b.title))
      .map((project) => project.url);
    if (!projectUrls.length) return catalog;
  ...
  async resolve(url: string): Promise<CatalogConversation | null> {
