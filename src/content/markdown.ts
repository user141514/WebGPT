import type { SemanticBlock, SemanticDocument, SemanticInline } from './model.js';

function escapeText(value: string): string {
  return value.replace(/\\/g, '\\\\').replace(/([*_\[\]])/g, '\\$1');
}

function inlineToMarkdown(inline: SemanticInline): string {
  if (inline.type === 'text') return escapeText(inline.text);
  ...
  if (inline.type === 'strong') return `**${inlinesToMarkdown(inline.content)}**`;
  if (inline.type === 'emphasis') return `*${inlinesToMarkdown(inline.content)}*`;
  return `~~${inlinesToMarkdown(inline.content)}~~`;
}

function inlinesToMarkdown(content: SemanticInline[]): string {
...
function tableCell(content: SemanticInline[]): string {
  return inlinesToMarkdown(content).replace(/\|/g, '\\|').replace(/\r?\n/g, '<br>');
}

function blockToMarkdown(block: SemanticBlock): string {
  if (block.type === 'paragraph') return inlinesToMarkdown(block.content);
  if (block.type === 'heading') return `${'#'.repeat(Math.min(6, Math.max(1, block.level)))} ${inlinesToMarkdown(block.content)}`;
...
export function semanticDocumentToMarkdown(document: SemanticDocument): string {
